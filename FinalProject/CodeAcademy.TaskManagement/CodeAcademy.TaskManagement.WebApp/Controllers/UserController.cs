...
namespace CodeAcademy.TaskManagement.WebApp.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public ActionResult Approve(string userId)
        {
            ...
            return RedirectToAction("PendingApproval", "Users");
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (user == null || !user.IsActive || user.PendingApproval)
            {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var pendingApproval = !User.IsInRole("Admin");

                var user = new User
                {
                    UserName = model.Email, Email = model.Email, IsActive = true,
                    DisplayName = model.DisplayName, PendingApproval = pendingApproval, DateCreated = DateTime.Now
                };
                ...
                if (result.Succeeded)
                {
                    ...
                    if (!pendingApproval && model.IsAdmin)
                    {
                    ...
                    return View("_SuccessInfo", 
                        new SuccessInfoVM
                        {
                            ...
                            Description = pendingApproval ? "Pending admin approval." : "Successfully registered new user"
                        });
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //TODO handle if sending was unsuccessful
                await UserManager.SendEmailAsync(user.Id, "Reset Password", "<h3>Task Management Application</h3>Please reset your password by clicking <a href=\"" + callbackUrl + "\">here</a>");
